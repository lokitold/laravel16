...
class Catalogo3 extends Command
{
    /**
...
//        $this->comment(PHP_EOL.'limit -> 50'.PHP_EOL);
        ...
        foreach ($dataLookBD as $dataLook):
            ...
            $dataCuponOtro = DB::table('cupone_otro')
                ->whereNotNull('nombres_apellidos')
                ->where('correo',$dataLook->correo)
                ->first();
            $this->commentArray($dataCuponOtro);

            if(!empty($dataCuponOtro->nombres_apellidos) and stripos($dataCuponOtro->nombres_apellidos,'null') === false):
                $this->commentArray($dataCuponOtro->correo);
                $this->commentArray($dataCuponOtro->nombres_apellidos);

                $dataUpdate = DB::table('oc_promocion_descuento')
                    ->where('catalogo', 'botas-y-chompas')
                    ->where('correo', $dataLook->correo)
                    ->update(['nombres_apellidos' => $dataCuponOtro->nombres_apellidos]);
